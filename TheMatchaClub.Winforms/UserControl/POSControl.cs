...
namespace TheMatchaClub.Winforms
{
    public partial class POSControl : UserControl
    {
        ...
        public POSControl()
        {
            ...
            // Apply double buffering to heavy panels to prevent flickering
        ...
        private void lstOrders_DoubleClick(object? sender, EventArgs e)
        {
            ...
            {
                MyUniversalBox.Show("Please enter a valid whole number for the quantity.", "Invalid Input", isError: true);
                return;
            }
        ...
        private async void POSControl_Load(object? sender, EventArgs e)
        {
            try
            {
                // Mapping Enums to Radio Button Tags
            ...
            {
                MyUniversalBox.Show("System failed to initialize POS: " + ex.Message, "System Error", isError: true);
            }
        }
        ...
        {
            if (keyData == Keys.Enter) { btnChoose_Click(this, EventArgs.Empty); return true; }
            if (keyData == Keys.Delete) { btnRemove_Click(this, EventArgs.Empty); return true; }
        ...
        private void RefreshOrderList()
        {
            ...
            foreach (var item in _cart)
            {
                ...
                total += subTotal;
                string line = item.Name.PadRight(18) + $" x{item.Quantity}".PadRight(6) + $"₱{subTotal}";
        ...
        private void RadioButton_CheckedChanged(object sender, EventArgs e)
        {
            ...
            switch (rb.Tag)
            {
                case PaymentMethod payment: _selectedPayment = payment; break;
                case OrderType orderType: _selectedOrderType = orderType; break;
            }
        }
        ...
        private void btnCheckout_Click(object sender, EventArgs e)
        {
            if (_cart.Count == 0)
            {
                MyUniversalBox.Show("The cart is empty. Please add items before checking out.", "Empty Cart", isError: false);
                return;
            }
            if (string.IsNullOrWhiteSpace(txtCustomer.Text))
            {
                MyUniversalBox.Show("Please enter a customer name to proceed.", "Information Missing", isError: false);
                return;
            }
        ...
        {
            if (lstOrders.SelectedIndex < 0) return;
            _cart.RemoveAt(lstOrders.SelectedIndex);
            RefreshOrderList();
        }
        ...
        {
            // Keeping standard MessageBox for the Yes/No confirmation (logic safety)
            var confirm = MessageBox.Show("Are you sure you want to end this session?", "End Session",
            ...
            try
            {
                ...
                MyUniversalBox.Show("Session has been closed successfully.", "Session Ended", isError: false);
            ...
            {
                MyUniversalBox.Show("Failed to close session: " + ex.Message, "System Error", isError: true);
            }
        }
        ...
        private async void btnStartSession_Click(object sender, EventArgs e)
        {
            ...
            {
                MyUniversalBox.Show("Please provide a name for this session (e.g., Morning Shift).", "Session Name Required", isError: false);
                return;
            }
            ...
            try
            {
                ...
                lblSession.Text = $"Active Session: {txtSessionName.Text}";
                pnlStartSession.Visible = false;
                pnlPOS.Visible = true;
            }
            catch (Exception ex)
            {
                MyUniversalBox.Show("Failed to start session: " + ex.Message, "Database Error", isError: true);
            }
        }
        ...
        {
            if (lstOrders.SelectedIndex < 0) return;
            _cart[lstOrders.SelectedIndex].Quantity++;
            RefreshOrderList();
        }
        ...
        private void btnDecrease_Click(object sender, EventArgs e)
        {
            ...
            var item = _cart[lstOrders.SelectedIndex];
            item.Quantity--;
            if (item.Quantity <= 0) _cart.RemoveAt(lstOrders.SelectedIndex);
            RefreshOrderList();
        }
    }
}